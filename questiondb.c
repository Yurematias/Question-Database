...
unsigned char menu(char* menu)
{
	...
	do
	{
		...
		while(1)
			...
			{
				tecla = getch();
				if((tecla == 'w' || tecla == 'w' || tecla == 72) && op > 1)	// 72 eh a seta para cima na tabela ASCII 
					op--;
				else if((tecla == 's' || tecla == 'S' || tecla == 80) && op < limite) // 80 eh a seta para baixo na tabela ASCII 
					op++;
